...
def save_model(args, epoch, model, metric_fc, optimizer):
    model_params_path = os.path.join(args.save_model, args.use_model, 'params', 'epoch_%d' % epoch)
    ...
    # 删除旧的模型
    old_model_path = os.path.join(args.save_model, args.use_model, 'params', 'epoch_%d' % (epoch - 3))
    ...
    for epoch in range(last_epoch, args.num_epoch):
        ...
        for batch_id, (img, label) in enumerate(train_loader()):
            ...
            if batch_id % 100 == 0 and dist.get_rank() == 0:
                ...
                train_step += 1
                loss_sum = []
            if batch_id % 20000 == 0 and batch_id != 0 and dist.get_rank() == 0:
